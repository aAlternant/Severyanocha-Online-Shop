...
function Cart({ items, onClickRemove, makeOrder, setCount }) {
    ...
    const getFullSummary = () => {
        if (items.length > 0) {
            return items
                .reduce((accum, current) => accum + current.price * (current.count ?? 1), 0)
                .toFixed(2);
        } else {
            return 0;
        }
    ...
    const getFullDiscount = () => {
        if (items.length > 0) {
            return items
                .reduce((accum, current) => accum + current.discount * (current.count ?? 1), 0)
                .toFixed(2);
        } else {
            return 0;
        }
    ...
    const getCurrentFullPrice = () => {
        if (items.length > 0) {
            return (getFullSummary() - getFullDiscount()).toFixed(2);
        } else {
            return 0;
        }
    };
        ...
        <div className="cart">
            ...
            <div className="totalyBlock">
                <div className="promotionBlock">
                    ...
                    <span className="ml-10" style={{ color: 'black' }}>
                        Списать 200 ₽{' '}
                    </span>
                    <p className="mt-20" style={{ color: '#8F8F8F' }}>
                        На карте накопленно 200 ₽
                    </p>
                </div>
                <div className="mathBlock">
                    <div className="d-flex justify-between align-center ml-10 mr-10 mt-25">
                        <span>
                            {items.length} {valueOfItems()}{' '}
                        </span>
                        {getFullSummary()} ₽
                    </div>
